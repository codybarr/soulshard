...
func activate(attack: AttackData, direction: Vector2, source: Node2D) -> void:
	_struck.clear()
	_damage = DamageData.new(
		attack.damage,
		source,
		direction * attack.knockback_force,
		attack.hit_stop_seconds,
	)
	position = direction * attack.hitbox_offset.x
